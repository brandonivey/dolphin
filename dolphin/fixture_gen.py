...
@fixture_generator(FeatureFlag, requires=["dolphin.test_users"])
def test_user_flags():
    ...
    ff = FeatureFlag.objects.create(name="", enabled=True, staff_only=True)
    ff.users.add(User.objects.get(username="registered"))
    ff.users.add(User.objects.get(username="staff"))
